...
app.get('/twitter', async (req, res) => {
    try {
        ...
        request_data.url = `https://api.twitter.com/1.1/statuses/user_timeline.json?screen_name=${username}&count=5`;
...
app.all('*', async (req, res) => {
    try {
        ...
        delete headers.host;
        if (!headers['target-url']) return res.status(420).json({error: 'Invalid target-url header'});
